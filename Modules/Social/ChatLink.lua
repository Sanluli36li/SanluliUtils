...
local Module = SanluliUtils:NewModule("social.chat.hyperlinkEnhance")
local L = SanluliUtils.Locale

local CONFIG_ENABLE = "enable"
local CONFIG_DISPLAY_ICON = "displayIcon"
local CONFIG_DISPLAY_ITEM_LEVEL = "displayItemLevel"
local CONFIG_DISPLAY_ITEM_TYPE = "displayItemType"
local CONFIG_DISPLAY_SOCKETS = "displaySockets"
local CONFIG_APPLY_TO_GUILD_NEWS = "applyToGuildNews"
...
local function HandleItemLink(itemLink)
    ...
    if tooltipInfo and tooltipInfo.type == Enum.TooltipDataType.Item and tooltipInfo.lines then
        for i, line in ipairs(tooltipInfo.lines) do
            ...
            elseif Module:GetConfig(CONFIG_DISPLAY_SOCKETS) and line.type == Enum.TooltipDataLineType.GemSocket then
                -- 插槽信息
    ...
    -- 物品分类
    if Module:GetConfig(CONFIG_DISPLAY_ITEM_TYPE) then
        local displayType
    ...
    if Module:GetConfig(CONFIG_DISPLAY_ITEM_LEVEL) and (
    ...
    -- 物品图标
    if Module:GetConfig(CONFIG_DISPLAY_ICON) and itemTexture then
        newItemLink = "|T"..itemTexture..":12:12:1:0|t"..newItemLink
    end

    -- 插槽图标
    if Module:GetConfig(CONFIG_DISPLAY_SOCKETS) then
...
local function chatFilter(chatFrame, event, message, ...)
    if not Module:GetConfig(CONFIG_ENABLE) then return end
    local newMessage = message
    :gsub("\124c[\\a-zA-Z0-9:]+\124Hitem:[^\124]+\124h%b[]\124h\124r", HandleItemLink)
    :gsub("(\124c[\\a-zA-Z0-9:]+\124Hkeystone:([0-9]+):[^\124]+\124h(%b[])\124h\124r)", function(link, itemIDStr, keystoneName)
        -- 史诗钥石
        if Module:GetConfig(CONFIG_DISPLAY_ICON) then
...
hooksecurefunc("GuildNewsButton_SetText", function(button, text_color, text, text1, text2, ...)
    if not Module:GetConfig(CONFIG_APPLY_TO_GUILD_NEWS) then return end
